...
public class EventParticipantService : IEventParticipantService
{
    ...
    public async Task<IEnumerable<ParticipantsByEvent>> GetAllEventParticipantByEvent(Guid id)
    {
        var partners = await _uow.ParticipantEventRepository.GetAllEventPartticipantByEvent(id);

        if (partners == null || !partners.Any())
            return Enumerable.Empty<ParticipantsByEvent>();

        try
        {
            var partnerDtos = _map.Map<IEnumerable<ParticipantsByEvent>>(partners);
            return partnerDtos;
        }
        catch (Exception ex)
        {
    ...
    public async Task<bool> DeleteAsync(Guid Id, Guid id)
    {
        var success = await _uow.ParticipantEventRepository.DeleteEventParticipantByEvent(Id, id);
        await _uow.CompleteAsync();
        return success;
    }
