...
public class RabbitMQServer
{
    ...
    {
        _service = service;
        var factory = new ConnectionFactory() { HostName = "srvprodjobintech.eastus.cloudapp.azure.com", Password = "adminmmc", UserName = "adminmmc" };
                        ...
                        if (dd.Length >= 3 && dd[1] == "Delete")
                        {
                            if (Guid.TryParse(dd[2], out Guid resultGuid))
                            {
                               // var deletionResult = await _service.EventParticipantService.DeleteAsync(resultGuid);
                                var deletionResult = await _service.ParticipantService.DeleteAsync(resultGuid);
                            }
                        }
                        else
                        {
