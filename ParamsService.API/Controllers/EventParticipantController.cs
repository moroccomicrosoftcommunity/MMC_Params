    ...
    public class EventParticipantController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateNewParticipantEvent(EventParticipantCreateCmd cmd)
        {
            try
            {
                ...
                //if (partner != null) 
                //{ 
                    var tt = await _notify.SendNotificationVia("Email", cmd.id_Participant.ToString(), cmd.id_Event.ToString()); 
                //}
        ...
        [HttpDelete("{Id}/{idparti}")]
        public async Task<IActionResult> Delete(Guid Id, Guid idparti)
        {
            var success = await _mediator.Send(new EventParticipantDeleteCmd(Id, idparti));
